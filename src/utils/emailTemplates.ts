...
export const sendContactNotificationEmail = (to: string | string[], data: ContactEmailProps) => {
    ...
    sendMail(to, `New Message: ${data.name} via Appon Islam Portfolio`, html);
};
 ...
 */
const renderBaseLayout = ({ preheader = "Notification from Appon Islam", title, bodyHtml }: { preheader?: string; title: string; bodyHtml: string }) => {
...
export const sendWelcomeEmail = (email: string, name: string) => {
    ...
    const bodyHtml = `
        ...
        <div style="text-align: center; margin: 32px 0;">
            <a href="${config.client_url || "https://www.apponislam.com"}" target="_blank" style="background-color: #10b981; color: #ffffff; font-weight: 700; font-size: 15px; text-decoration: none; padding: 14px 32px; border-radius: 8px; display: inline-block;">
