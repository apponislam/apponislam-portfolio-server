...
export const parseActionOrTypeFilter = (input?: ActivityType | ActivityType[] | string): any => {
    ...
    for (const item of items) {
        ...
        if (ACTIVITY_CATEGORIES[upper]) {
            ...
            const match = Object.values(ActivityType).find((act) => act.toUpperCase() === upper);
            if (match) {
...
const deleteActivity = async (activityId: string) => {
    const activity = await ActivityModel.findOneAndUpdate({ _id: activityId, isDeleted: false }, { $set: { isDeleted: true } }, { returnDocument: "after" });
